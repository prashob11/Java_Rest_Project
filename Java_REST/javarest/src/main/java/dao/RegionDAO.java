...
public class RegionDAO {
	...
	public List<Region> getRegionsCascadingList(String countryId, String regionId) {
		...
		if (!StringUtils.isNullOrEmpty(regionId)) {
			...
			Optional<Region> selectedRegion = regions.stream().filter(r -> r.getRegionId() == rId).findFirst();

			/*
			 * if region with given regionId is in the list then move it in the beginning of
			 * the list (for Edit page)
			 */
